...
GEMMA_LLM_KEY_OPS = (
    ...
    # transformers <5.x nested the ViT under vision_tower.vision_model.*; 5.x
    # flattened it to vision_tower.*. Strip that infix first (if present) so the
    # checkpoint's real vision weights load onto the flattened 5.x params instead
    # of being left on the meta device. (Rebels local patch for transformers 5.x)
    .with_replacement("vision_tower.vision_model.", "vision_tower.")
...
def create_and_populate(module: GemmaTextEncoder) -> GemmaTextEncoder:
    model = module.model
    # transformers <5.x: SiglipVisionModel wraps the transformer as .vision_model.
    # transformers >=5.x: SiglipVisionModel is flattened - embeddings/encoder live
    # directly on it, so the vision_tower IS the object with .embeddings.
    # (Rebels local patch for transformers 5.x)
    _vision_tower = model.model.vision_tower
    v_model = getattr(_vision_tower, "vision_model", _vision_tower)
    l_model = model.model.language_model

    # Text-only Gemma checkpoints carry no SigLIP vision_tower weights, so those
    # params never leave the meta device and the ledger's later .to(device)
    # raises "Cannot copy out of meta tensor". The text encode path never runs
    # the vision tower, so materialize it as empty CPU tensors: for a text-only
    # checkpoint they stay unused; for a full multimodal checkpoint the
    # subsequent load_state_dict(assign=True) overwrites them with the real
    # weights. (Rebels local patch for transformers 5.x)
    if any(p.is_meta for p in _vision_tower.parameters()):
        _vision_tower.to_empty(device="cpu")

    config = model.config.text_config
    dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
    base = config.rope_local_base_freq
    local_rope_freqs = 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.int64).to(dtype=torch.float) / dim))
    # transformers <5.x: rope_scaling is a flat dict with "rope_type".
    # transformers >=5.x: it is a per-layer dict {"full_attention": {...}, "sliding_attention": {...}}
    # and the init function requires layer_type. "default" rope has NO entry in
    # ROPE_INIT_FUNCTIONS on 5.x - fall back to the plain inv_freq formula.
    # (Rebels local patch for transformers 5.x)
    def _default_inv_freq(theta):
        return 1.0 / (theta ** (torch.arange(0, dim, 2, dtype=torch.int64).to(dtype=torch.float) / dim))
    _rs = getattr(config, "rope_scaling", None) or {}
    if isinstance(_rs, dict) and "rope_type" in _rs:
        _rt = _rs["rope_type"]
        if _rt in ROPE_INIT_FUNCTIONS:
            inv_freqs, _ = ROPE_INIT_FUNCTIONS[_rt](config)
        else:  # "default" or unknown
            inv_freqs = _default_inv_freq(float(getattr(config, "rope_theta", 1000000.0)))
    else:
        _full = _rs.get("full_attention", {}) if isinstance(_rs, dict) else {}
        _rt = _full.get("rope_type", "linear")
        if _rt in ROPE_INIT_FUNCTIONS:
            inv_freqs, _ = ROPE_INIT_FUNCTIONS[_rt](config, layer_type="full_attention")
        else:  # "default" or unknown
            inv_freqs = _default_inv_freq(float(_full.get("rope_theta", getattr(config, "rope_theta", 1000000.0))))
    ...
    if hasattr(l_model, "rotary_emb_local"):
        # transformers <5.x: separate local (sliding) and global rotary modules.
        l_model.rotary_emb_local.register_buffer("inv_freq", local_rope_freqs)
        l_model.rotary_emb.register_buffer("inv_freq", inv_freqs)
    else:
        # transformers >=5.x: ONE Gemma3RotaryEmbedding with per-layer-type buffers:
        # sliding_attention_* == old rotary_emb_local (base rope_local_base_freq),
        # full_attention_*    == old rotary_emb (scaled/global rope).
        # (Rebels local patch for transformers 5.x)
        _rot = l_model.rotary_emb
        _rot.register_buffer("sliding_attention_inv_freq", local_rope_freqs, persistent=False)
        _rot.register_buffer("sliding_attention_original_inv_freq", local_rope_freqs.clone(), persistent=False)
        _rot.register_buffer("full_attention_inv_freq", inv_freqs, persistent=False)
        _rot.register_buffer("full_attention_original_inv_freq", inv_freqs.clone(), persistent=False)
